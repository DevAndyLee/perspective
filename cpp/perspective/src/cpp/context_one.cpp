...
/**
 * @brief Returns row indices that have been updated with new data.
 *
 * @return t_rowdelta
 */
t_rowdelta
t_ctx1::get_row_delta() {
    ...
    auto aggidxs = m_tree->get_delta_ridxs();
    rows.reserve(aggidxs.size());
    std::copy(aggidxs.begin(), aggidxs.end(), std::back_inserter(rows));
